...
class NewSkillDialog(toga.Window):
    def __init__(self, title: str, focus_app: "FocusApp"):
        ...
        secondary_stat_label = toga.Label(
            "Secondary stat:", style=Pack(padding=(0, 5))
        )
...
class LevelUpDialog(toga.Window):
    def __init__(self, title: str, skill, previous_level: int, xp_gained: int):
        ...
        self.content = toga.Box(style=Pack(direction=COLUMN, padding=20, alignment=CENTER))
        
        # Header
        level_label = toga.Label(
            f"🎉 Level Up! 🎉",
            style=Pack(font_size=20, font_weight="bold", padding_bottom=15, alignment=CENTER)
        )
        
        # Skill name and level progression
        skill_label = toga.Label(
            f"{skill.name.title()}",
            style=Pack(font_size=16, font_weight="bold", padding_bottom=5, alignment=CENTER)
        )
        level_progress_label = toga.Label(
            f"Level {previous_level} → Level {skill.level}",
            style=Pack(font_size=14, color="#2E7D32", padding_bottom=15, alignment=CENTER)
        )
        
        # XP gained
        xp_label = toga.Label(
            f"🌟 +{xp_gained} XP gained!",
            style=Pack(font_size=12, color="#1976D2", padding_bottom=10, alignment=CENTER)
        )
        ...
        stats_header = toga.Label(
            "📈 Stats Increased:",
            style=Pack(font_size=12, font_weight="bold", padding_bottom=5, alignment=CENTER)
        )
        ...
        progress_box = toga.Box(style=Pack(direction=COLUMN, padding=10, alignment=CENTER))
        ...
        progress_bar = toga.ProgressBar(
            ...
            style=Pack(padding=(5, 20), width=300)
        )
...
class FocusApp(toga.App):
    ...
    def _on_level_gained(self, skill, previous_level, xp_gained):
        ...
        asyncio.create_task(self.level_gained(skill, previous_level, xp_gained))
    ...
    def _create_skill_box(self, skill):
        ...
        stats_label = toga.Label(
            stats,
            style=Pack(font_size=11, color="#888888", padding_top=5)
        )
        ...
        progress_bar = toga.ProgressBar(
            max=1.0,
            value=current_progress,
            style=Pack(padding=(5, 0), width=200)
        )
        ...
        skill_container = toga.Box(
            style=Pack(direction=COLUMN, padding=(5, 10))
        )
    ...
    def _create_stat_card(self, stat):
        ...
        progress_bar = toga.ProgressBar(
            max=1.0,
            value=progress,
            style=Pack(padding=(5, 0), width=250)
        )
        ...
        stat_container = toga.Box(
            style=Pack(direction=COLUMN, padding=(3, 8))
        )
    ...
    def _get_stat_info(self, stat_name):
        ...
        return stat_configs.get(stat_name.lower(), {
            "max": 20,
            "icon": "📊",
            "description": "Character attribute",
            "colors": {
                "low": {"bg": "#f5f5f5", "accent": "#616161", "text": "#424242", "secondary": "#757575"},
                "medium": {"bg": "#eeeeee", "accent": "#424242", "text": "#212121", "secondary": "#616161"}, 
                "high": {"bg": "#e0e0e0", "accent": "#212121", "text": "#000000", "secondary": "#424242"}
            }
        })
    ...
    def _create_goals_box(self):
        ...
        goals_scroll = toga.ScrollContainer(
            content=goals_list_box,
            style=Pack(height=300, padding=5)
        )
    ...
    def _create_timer_box(self):
        ...
        self.break_progress = toga.ProgressBar(
            max=1.0,
            value=0.0,
            style=Pack(width=300, padding=(10, 20))
        )
    ...
    async def add_skill(self, widget):
        ...
        if result == "Save":
            ...
            self.skills_list_box.add(self._create_skill_box(self.focus_app.new_skills[dialog.name_input.value]))
            self.skill_selection.items = [skill.name.title() for skill in self.focus_app.new_skills.values()]
    ...
    async def _update_timers(self):
        ...
        while True:
            ...
            duration = duration_from_seconds(
            ...
            if self.focus_app.focusing:
                ...
                if self.focus_app.earned_break_time > 0:
                    break_progress = min(current_break_time / max(self.focus_app.earned_break_time, 1), 1.0)
